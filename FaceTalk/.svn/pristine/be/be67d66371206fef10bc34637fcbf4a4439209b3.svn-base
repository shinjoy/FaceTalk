...
public class QnaDao {
	...
	private RowMapper qnaMapper = new RowMapper() {
		public Qna mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			qna.setSite(rs.getString("site"));
			return qna;
		}
	};
	private RowMapper VqnaMapper = new RowMapper() {
		public Qna mapRow(ResultSet rs, int rowNum) throws SQLException {
			Qna qna = new Qna();
			qna.setNoticeSeq(rs.getInt("notice_seq"));
			qna.setUserId(rs.getString("user_id"));
			...
			qna.setrRegDate(rs.getString("r_reg_date"));
			qna.setSite(rs.getString("site"));
			qna.setSiteName(rs.getString("site_name"));
			return qna;
		}
	};
	public void addQna(final Qna qna) {
		String query = "" +
				...
				+ " reg_date, answer,r_level,"
				+ " r_comment_seq, r_reg_date,site) " +
				"VALUES " +
				"	(?, ?, ?,"
				+ " ?, ?, ?,"
				+ " ?, ?, ?,"
	...
	public List<Qna> getQnaList(String site,int userType,String keyword, int page, int itemCountPerPage) {
		
		String con=" where r_level = 0 ";
		if(userType!=1){
			con += " AND site = '"+ site +"' ";
		}
		if(!keyword.equals("")){
		...
		String query = ""
				...
				+ itemCountPerPage + " ";
		return (List<Qna>) this.jdbcTemplate.query(query, this.VqnaMapper);
	}
