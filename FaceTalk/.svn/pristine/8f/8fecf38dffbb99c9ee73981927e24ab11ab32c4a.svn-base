			...
			<c:choose>
				<c:when test="${list.size() > 0}">
					<c:forEach var="it" items="${list}">
								...
								<tr>
									<td class="text-left" >${it.userSeq}</td>
									...
									<td class="text-left"><a href="/admin/user/user_view.go?userId=${it.userId}">${it.userId}</a></td>
									<td class="text-left">${it.phoneNumber}</td>
									<td class="text-left"><a href="/admin/user/user_view.go?userId=${it.userId}">${it.userName}</a></td>
									<td>${it.userAge}</td>
									<td>${it.genderText}</td>
									<td>${it.area}</td>
