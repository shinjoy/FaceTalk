...
<script type="text/javascript">
	...
	function deleteImg(contentsHtml) {
		if(confirm("이미지를 삭제하시겠습니까?")) {
			...
			$("#imgdiv").remove();
			var str = '<input type="file" name="imgFile" > ';
				str+='<input type="hidden" name="delfile" value="'+contentsHtml+'">';
	...
	<section id="main">
		...
		<section id="contents">
			...
			<div class="contents-block">
									...
									<c:choose>
										...
										<c:otherwise>
											<div id="imgdiv">
												...
												<button type="button" class="btn tiny" onclick="deleteImg('${notice.contentsHtml}');">삭제</button>	
											</div>
											<div id="img-in"></div>
										</c:otherwise>
									</c:choose>
								...
								<td>
									...
									<c:choose>
										<c:when test="${USER_TYPE==1}">
											<tr>
												<th>사이트</th>
												<td class="radio"><c:forEach items="${list}" var="it">
														<label><input type="radio" name="site"
															value="${it.serviceId}"
															${notice.site == it.serviceId ? 'checked=\"checked\"' : ''}>${it.siteName}</label>
													</c:forEach></td>
											</tr>
										</c:when>
										<c:otherwise>
				...
				<div class="btn-tools">
					<button type="submit" class="btn-blue">팝업공지저장</button>
