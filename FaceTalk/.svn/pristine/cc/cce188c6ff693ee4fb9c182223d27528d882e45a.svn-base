...
function readContents(seq, obj) {
	...
	$(".open .contents").html("");
	$(".open").animate({height:68},100,null);
	...
	if ($(obj).hasClass("open")) {
		...
		$(obj).animate({height:68},100,null);
		...
		$.ajax({
			...
			success:function(msg){
				...
				$(obj).animate({height:$(obj).children(".contents").outerHeight(true)+40},200,function() {
				//$(obj).animate({height:450},200,function() {
					$('html,body').animate({scrollTop:$(obj).offset().top}, 500);
				});
				$(obj).children(".contents-arrow").html("<img src=\"/images/bullet_arrow_down.png\" class=\"arrow\" style=\"width:30px;height:30px;border-left:0;\">");
				$(obj).addClass("open");
