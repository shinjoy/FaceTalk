...
/*	private RowMapper messageMapper = new RowMapper() {
	...
	private RowMapper messageMapper = new RowMapper() {
		public Message mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			message.setFriendBirthyear(rs.getInt("friend_birthyear"));
			message.setMessage(rs.getString("message"));
			message.setAgentId(rs.getString("agent_id"));
			return message;
		}
	};
	...
	public List<Message> getMessageList(String user_id , int page, int itemCountPerPage) {
		...
		return (List<Message>)this.jdbcTemplate.query(query, new Object[] { user_id,user_id}, this.messageMapper);
	}

	//알림내역 리스트 갯수
	public int getCount(String user_id) {
		String query = " SELECT count(*) FROM V_NF_MESSAGE WHERE user_id = ? or agent_id = ? ";
		return this.jdbcTemplate.queryForInt(query, new Object[] { user_id,user_id });
	}

	//알림내역 리스트dd
	public List<Message> getMessageListAll(String user_id) {
