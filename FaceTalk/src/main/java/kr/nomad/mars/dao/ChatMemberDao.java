...
public class ChatMemberDao {
	...
	private RowMapper vChatmemberMapper = new RowMapper() {
		public ChatMember mapRow(ResultSet rs, int rowNum) throws SQLException {
			ChatMember chatmember = new ChatMember();
			chatmember.setChatMemberSeq(rs.getInt("chat_member_seq"));
			chatmember.setChatRoomSeq(rs.getInt("chat_room_seq"));
			...
			chatmember.setGroupId(rs.getString("group_id"));
			chatmember.setNotification(rs.getInt("notification"));
			chatmember.setAgentId(rs.getString("agent_id"));
			return chatmember;
		}
	};
	...
	public ChatMember getChatMember(int chat_member_seq) {
		String query = "" + 
				"SELECT chat_member_seq, chat_room_seq, user_id, reg_date, group_id " +
				"FROM T_NF_CHAT_MEMBER " +
				"WHERE chat_member_seq = ? ";
		return (ChatMember)this.jdbcTemplate.queryForObject(query, new Object[] { chat_member_seq }, this.chatmemberMapper);
	}

	public List<ChatMember> getChatMemberList(int roomSeq) {
		String query = "" + 
				"SELECT * " +
				"FROM V_NF_CHAT_MEMBER " +
				"WHERE chat_room_seq = ? ";
		return (List<ChatMember>)this.jdbcTemplate.query(query, new Object[] { roomSeq }, this.vChatmemberMapper);
	}
	
	public ChatMember getChatMember(int chatRoomSeq, String agentId) {
		String query = "" + 
				"SELECT top 1 * " +
				"FROM V_NF_CHAT_MEMBER " +
				"WHERE chat_room_seq = ? AND agent_id = ? ";
		try {
			return (ChatMember)this.jdbcTemplate.queryForObject(query, new Object[] { chatRoomSeq, agentId }, this.vChatmemberMapper);
		} catch (Exception e) {
