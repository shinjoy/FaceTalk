				...
				<c:when test="${list.size() > 0}">
					<c:forEach var="it" items="${list}">
						<tr>
							...
							<td class="text-left"><a href="/pc/chat_user_edit.go?userId=${it.userId}&userSeq=${it.userSeq}">${it.userId}</a></td>
							<td class="text-left">${it.phoneNumber}</td>
							<td class="text-left"><a href="/pc/chat_user_edit.go?userId=${it.userId}&userSeq=${it.userSeq}">${it.userName}</a></td>
							<td>${it.userAge}</td>
							<td>${it.genderText}</td>
							<td>${it.area}</td>
							<td>${fn:substring(it.regDate,0,16)} </td>
							<td><button type="button" onclick="useThisId('${it.userId}','${it.userName}');" class="btn-green">사용</button>
						</tr>
					</c:forEach>
					</c:when>
					<c:otherwise>
