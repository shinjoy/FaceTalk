									...
									<div style="width:100%; padding-top:10px; background-color:#fff;">
											...
											<td>
												...
												<%-- <c:choose> --%>
												 		<%-- <c:when test="${it.goodStatus !=1 }"> --%>
															<img id='goodImg${it.bbsSeq}' src=${it.goodStatus !=1 ? '/images/btn_good_dw.png' : '/images/btn_good_big.png' } style="height:12px;" onclick="bbsGood('${IMG_ID}','${it.bbsSeq}')">
														<%-- </c:when>
												 		<c:otherwise>
															<img src= style="height:12px;" onclick="bbsGood('${IMG_ID}','${it.bbsSeq}')"> 
												 		
												 		</c:otherwise> --%>
												<%--  </c:choose> --%>
												 <span id="good${it.bbsSeq}">${it.likeCount}</span>		
												</span>
												<span class="good"><img src='/images/btn_reply_dw.png' style="height:12px;"> ${it.commentCount}</span>
												
												 
												 <c:if test="${it.userId ==IMG_ID}">	
													<button  class="btn" onclick="location.href='/pc/talk_edit.go?userId=${IMG_ID}&bbsSeq=${it.bbsSeq}';return false">수정</button>
										...
										<c:if test="${it.kind==2 }">
											<div class="shareBox" style="border:2px solid gold;">
												...
												<hr>
												<a href="/pc/talk_view.go?bbsSeq=${it.shareSeq}&userId=${IMG_ID}">
												<div style="padding:10px; border-top:1px solid #aaa;">
													<c:choose>
