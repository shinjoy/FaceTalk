  ...
  (function() {
	...
	function formCheck(formData, frm, options) {
		...
		if (firmForm.agentId.value == "") {
			...
			alert("이름을 입력해주세요.");
			return false;
		} else if (firmForm.gender[0].checked == false
				&& firmForm.gender[1].checked == false) {
			alert("성별을 선택해주세요.");
			return false;
	...
	function formSuccess(responseText, statusText, xhr, $form) {
		...
		try {
			if (json.result == false) {
				...
				document.location.href = "/admin/user/user_imagine.go";
			}
		} catch (e) {
			alert(json.message);
		}
...
<section class="main-cover main-row">
	<section id="main">
		...
		<section id="contents">
			...
			<form method="post" name="firmForm" id="firmForm" enctype="multipart/form-data" action="/admin/user/user_imagine_edit_do.go">
			<input type="hidden" name="userSeq" value="${userSeq}">
				...
				<div class="contents-block">
					...
					<tr>
						...
						<td>
							<select name="agentId">
								<option value="">채팅관리자 선택</option>
								<c:forEach var="it" items="${chatUserList}">
									<option value="${it.userId}" ${it.userId == user.agentId ? 'selected=\"selected\"' : '' }>${it.userName} [${it.userId}]</option>
								</c:forEach>
							</select>
						</td> 
					</tr>
					<tr>
						<th><div class="icon">가상회원 닉네임</div></th>
						<td><input type="text" class="itext" name="userName"  value="${user.userName}" maxlength="8">(8자 이하로 입력해주세요.)</td> 
					</tr>
					<tr>
						<th>가상회원 성별</th>
						<td>										
					...
					<tr>
						...
						<c:choose>
							<c:when test="${userSeq == 0}">
								<td><input type="password" class="itext" name="password" maxlength="8">(8자 이하로 입력해주세요.)</td>
							</c:when>
							<c:otherwise>
					...
					<tr>
							...
							<td>
								...
								<div id='map' style="width:700px; height:400px; margin-top:10px;"><br></div>
								<div class="map-info">
				...
				<div  class="btn-tools" >
					...
					<button type="button" class="btn" onclick="document.location.href='/admin/user/user_imagine.go';">목록</button>
				</div>
